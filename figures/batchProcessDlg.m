function varargout = batchProcessDlg(varargin)
    %batchProcessDlg(varargin)
    ...
    % Edit the above text to modify the response to help batchProcessDlg
    
    % Last Modified by GUIDE v2.5 19-Jan-2019 09:34:51
    
    % Begin initialization code - DO NOT EDIT
    gui_Singleton = 1;
    gui_State = struct('gui_Name',       mfilename, ...
        'gui_Singleton',  gui_Singleton, ...
        'gui_OpeningFcn', @batchProcessDlg_OpeningFcn, ...
        'gui_OutputFcn',  @batchProcessDlg_OutputFcn, ...
        'gui_LayoutFcn',  [] , ...
        'gui_Callback',   []);
...
% --- Executes just before batchProcessDlg is made visible.
function batchProcessDlg_OpeningFcn(hObject, eventdata, handles, varargin)
    % This function has no output args, see OutputFcn.
    % hObject    handle to figure
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    % varargin   command line arguments to batchProcessDlg (see VARARGIN)
    ...
    if(isempty(MARKING))
        ...
        userdata.channel_h = handles.push_psd_channel;
        userdata.settings_h = handles.push_psd_settings;
        
        set(handles.menu_spectral_method,'userdata',userdata,'callback',{@menu_spectral_method_Callback,[handles.push_psd_channel,handles.push_psd_settings,handles.text_psd_channelSelection]},...
            'enable','off');
        ...
        if(numel(varargin)>1)
            ...
            updateSpectralSelection(handles.user.panel_psd{1}); 
        end
        
       
        % Choose default command line output for batchProcessDlg
...
function varargout = batchProcessDlg_OutputFcn(hObject, eventdata, handles)
...
function [pathname, BATCH_PROCESS,playlist] = getBatchSettings(handles)
    ...
    synth_channel_settings_lite = repmat(struct('channel_labels',{}),numel(synth_channel_names),1);
    ...
    numSpectralJobs = numel(psd_spectral_methods_h);
    spectral_settings = repmat(struct('method_label','','channel_indices',[],...
        'channel_labels',{},'reference_channel_label','','reference_channel_index',[],'params',struct()),numSpectralJobs,1);
    
    for k=1:numSpectralJobs                
    ...
    BATCH_PROCESS.spectral_settings = spectral_settings;
    ...
    templateStruct = struct('numConfigurations',1,'save2img',false,...
        'channel_labels',{},'channel_configs',{},'method_label','',...
        'method_function','','batch_mode_label','',...
        'pBatchStruct',struct(),'rocStruct',[],...
        'detectorID',[],...
        'params',struct(),'configID',NaN);
    
    event_settings = repmat(templateStruct,num_selected_events,1);    
...
function handles = createGlobalTemplate(handles)
    ...
    set([handles.menu_event_channel1
    ...
    handles.user.panel_psd = {[handles.menu_spectral_method
        handles.push_psd_channel
        handles.push_psd_settings
        handles.text_psd_channelSelection]};
    ...
    set([handles.edit_synth_CHANNEL_name
        handles.push_psd_channel
        handles.text_psd_channelSelection],'enable','off');
    ...
    src.text_channel_selection = get(handles.text_psd_channelSelection);
...
function push_add_psd_Callback(hObject, eventdata, handles)
    ...
    updateSpectralSelection(addedH);
...
function menu_spectral_method_Callback(hObject, ~, rowHandles)
    % hObject    handle to menu_spectral_method (see GCBO)
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    updateSpectralSelection([hObject, rowHandles]);
end

function updateSpectralSelection(psdRowHandles)
    global MARKING;
    PSD = MARKING.SETTINGS.PSD;
    MUSIC = MARKING.SETTINGS.MUSIC;
    
    menu_selection_h = psdRowHandles(1);
    channel_selection_h = psdRowHandles(2);
    settings_h = psdRowHandles(3);
    
    % set(channel_selection_h,'callback',
    spectralSelection = getSelectedMenuString(menu_selection_h);
    switch(lower(spectralSelection))
        case 'none'
            %disable
            set(psdRowHandles(2:end),'enable','off');            
        case 'psd'
            %enable channel selection
            %enable settings
            set(psdRowHandles(2:end),'enable','on');
...
function figure1_CloseRequestFcn(hObject, eventdata, handles)
    ...
    try
        ...
        % now.
        BATCH_PROCESS = handles.user.BATCH_PROCESS;
        multipleEntryFields = {'spectral_settings','event_settings','artifact_settings','synth_CHANNEL'};
